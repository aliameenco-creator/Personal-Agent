...
const DEFAULT_SYSTEM_PROMPT = `Role: You are the Lead Content Strategist for Ali Amin, a high-level AI Automation Engineer and founder of the AI Engineer Academy.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { topic, slideIndex, totalSlides } = req.body;

  if (!topic || slideIndex === undefined || !totalSlides) {
    return res.status(400).json({ error: 'Missing required fields: topic, slideIndex, totalSlides' });
  }

  const systemPrompt = await getPrompt('svg-carousel', DEFAULT_SYSTEM_PROMPT);
...
- Output ONLY the raw SVG code starting with <svg and ending with </svg>. No markdown, no code fences, no explanation.`;
  ...
  try {
    ...
    if (!svgMatch) {
      return res.status(422).json({ error: 'AI did not return valid SVG code' });
    }

    return res.json({ svg: svgMatch[0] });
  } catch (error: any) {
    return res.status(500).json({ error: error.message || 'SVG generation failed' });
  }
}
